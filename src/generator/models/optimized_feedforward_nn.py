...
class OptimizedBatchFeedForwardNN:
    def __init__(self):
        self.model = None
        self.vocab = None
        self.name = "Optimized FeedForwardNN"
    ...
    def _build_vocabulary(
        self, ingredients: List[List[str]], steps: List[List[str]]
    ) -> None:
        print("Building vocabulary...")
        all_tokens = [token for lst in ingredients + steps for token in lst] + [
            "<UNKNOWN>",
            "<s>",
            "</s>",
            "<STEPS>",
        ...
        self.vocab = [
            token for token, _ in sorted(counter.items(), key=lambda x: (-x[1], x[0]))
        ]
    ...
    def _prepare_train(
        ...
        for i in range(len(ingredients)):
            ingredient_ids = (
                [self.token_to_id["<s>"]]
                + [
                    self.token_to_id.get(token, self.token_to_id["<UNKNOWN>"])
                    for token in ingredients[i]
                ]
                + [self.token_to_id["<STEPS>"]]
            )
            step_ids = [
                self.token_to_id.get(step_token, self.token_to_id["<UNKNOWN>"])
                for step_token in steps[i]
            ] + [self.token_to_id["</s>"]]
    ...
    def fit(
        ...
        self.model.add(
            keras.layers.Embedding(
                vocab_size * context, embedding_dim, name="embedding"
            )
        )
        ...
        train_ds = (
            train_dataset.take(train_size).batch(batch_size).prefetch(tf.data.AUTOTUNE)
        )
        val_ds = (
            train_dataset.skip(train_size).batch(batch_size).prefetch(tf.data.AUTOTUNE)
        )
    ...
    def _prepare_test(self, ingredients: List[List[str]]):
        ...
        for ingredient_list in ingredients:
            # Convert ingredients to token IDs
            ingredient_ids = [
                self.token_to_id.get(token, self.token_to_id["<UNKNOWN>"])
                for token in ingredient_list
            ] + [self.token_to_id["<STEPS>"]]

            # Get the last context_size-1 tokens, pad with UNKNOWN if needed
            if len(ingredient_ids) >= self.context:
                # If we have enough tokens, take the last context_size
                context_tokens = ingredient_ids[-self.context :]
            else:
                # Pad with UNKNOWN tokens if we don't have enough
                padding = [self.token_to_id["<UNKNOWN>"]] * (
                    self.context - len(ingredient_ids)
                )
                context_tokens = padding + ingredient_ids

            # Create a single context for each ingredient list
            all_contexts.append(context_tokens)

        # Return as a list, not numpy array (we'll convert to batch arrays in predict)
        return all_contexts

    def predict(self, input_text: List[List[str]], max_length=100, temperature=None) -> List[List[str]]:
        ...
        completed = [False] * num_ingredients  # Track which sequences are completed
        ...
        for step in range(max_length):
            ...
            active_indices = [
                i for i, is_complete in enumerate(completed) if not is_complete
            ]
            ...
            for batch_start in range(0, len(active_indices), self.batch_size):
                        ...
                        if len(predicted_tokens) >= self.context:
                            token_ids = [
                                self.token_to_id.get(
                                    token, self.token_to_id["<UNKNOWN>"]
                                )
                ...
                if batch_input.shape != expected_shape:
                    if (
                        len(batch_input.shape) == 2
                        and batch_input.shape[1] == self.context
                    ):
                        pass
                    else:
                        try:
                            ...
                            padded_batch = np.zeros(expected_shape, dtype=int)
                            for i, context in enumerate(batch_contexts):
                                context_array = np.array(context, dtype=int)
                                if len(context_array) >= self.context:
                                    padded_batch[i] = context_array[-self.context :]
                                else:
                                    padded_batch[i, -len(context_array) :] = (
                                        context_array
                                    )
                ...
                for i, pred_idx in enumerate(batch_indices):
                    prediction = (
                        predictions[i] if len(predictions) > 1 else predictions[0]
                    )
    ...
    def evaluate(self, ingredients: List[List[str]], steps: List[List[str]]):
        ...
        ds = tf.data.Dataset.from_tensor_slices((X, y, weights)).shuffle(
            buffer_size=10000
        )
